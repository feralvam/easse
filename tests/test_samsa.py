...
def test_sentence_samsa():
    ...
    _, samsa_score = sentence_samsa(orig_sent, sys_sent, lowercase=True)
    assert samsa_score == pytest.approx(100.0)

    orig_sent = ("The river is indeed an ever-present part of the city's decor , and the official entrance "
                     "to Lisbon is a broad marble stair mounting from the water to the vast , "
                     "arcaded Commerce Square ( Praca do Comercio) .")
    sys_sent = ("The river is indeed an ever-present part of the city's decor , and the entrance to Lisbon "
    ...
    sys_sent = ("The incident followed the killing in August by Israeli soldiers. "
...
def test_corpus_samsa():
    ...
    _, samsa_score = corpus_samsa(orig_sents, refs_sents, lowercase=True, verbose=True)
