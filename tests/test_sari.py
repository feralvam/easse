...
def test_sari_corpus_plain():
    orig_sents = read_file("tests/data/turkcorpus/test.8turkers.tok.norm")
    for n in range(8):
        ref_lines = read_file(f"tests/data/turkcorpus/test.8turkers.tok.turk.{n}")
        if n == 0:
            ref_sents = [[line] for line in ref_lines]
        else:
            ref_sents = [x + [y] for x, y in zip(ref_sents, ref_lines)]

    hyp_sents = read_file("tests/data/systemoutputs/lower/Dress-Ls.lower")
    ...
    hyp_sents = read_file("tests/data/systemoutputs/lower/Dress.lower")
    sari_score = sari.sari_corpus(orig_sents, hyp_sents, ref_sents, tokenizer='plain')
    assert sari_score == pytest.approx(0.365859900146575)

    hyp_sents = read_file("tests/data/systemoutputs/lower/EncDecA.lower")
    sari_score = sari.sari_corpus(orig_sents, hyp_sents, ref_sents, tokenizer='plain')
    assert sari_score == pytest.approx(0.3473946658449856)

    hyp_sents = read_file("tests/data/systemoutputs/lower/Hybrid.lower")
    sari_score = sari.sari_corpus(orig_sents, hyp_sents, ref_sents, tokenizer='plain')
    assert sari_score == pytest.approx(0.31008109926854227)

    hyp_sents = read_file("tests/data/systemoutputs/lower/PBMT-R.lower")
    sari_score = sari.sari_corpus(orig_sents, hyp_sents, ref_sents, tokenizer='plain')
    assert sari_score == pytest.approx(0.37817966679481013)

    hyp_sents = read_file("tests/data/systemoutputs/lower/SBMT-SARI.lower")
    sari_score = sari.sari_corpus(orig_sents, hyp_sents, ref_sents, tokenizer='plain')
