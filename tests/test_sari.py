...
def test_corpus_sari_plain():
    ...
    ref_sents = []
    for n in range(8):
        ref_lines = read_lines(DATA_DIR / f"test_sets/turk/test.8turkers.tok.turk.{n}")
        ref_sents.append(ref_lines)

    hyp_sents = read_lines(DATA_DIR / "system_outputs/turk/lower/Dress-Ls.tok.low")
    ...
    hyp_sents = read_lines(DATA_DIR / "system_outputs/turk/lower/Dress.tok.low")
    sari_score = sari.corpus_sari(orig_sents, hyp_sents, ref_sents, tokenizer='plain')
    assert sari_score == pytest.approx(36.5859900146575)

    hyp_sents = read_lines(DATA_DIR / "system_outputs/turk/lower/EncDecA.tok.low")
    sari_score = sari.corpus_sari(orig_sents, hyp_sents, ref_sents, tokenizer='plain')
    assert sari_score == pytest.approx(34.73946658449856)

    hyp_sents = read_lines(DATA_DIR / "system_outputs/turk/lower/Hybrid.tok.low.low")
    sari_score = sari.corpus_sari(orig_sents, hyp_sents, ref_sents, tokenizer='plain')
    assert sari_score == pytest.approx(31.008109926854227)

    hyp_sents = read_lines(DATA_DIR / "system_outputs/turk/lower/PBMT-R.tok.low")
    sari_score = sari.corpus_sari(orig_sents, hyp_sents, ref_sents, tokenizer='plain')
    assert sari_score == pytest.approx(37.817966679481013)

    hyp_sents = read_lines(DATA_DIR / "system_outputs/turk/lower/SBMT-SARI.tok.low.low")
    sari_score = sari.corpus_sari(orig_sents, hyp_sents, ref_sents, tokenizer='plain')
