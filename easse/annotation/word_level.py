    ...
    for ref_token in ref_annots:
        ...
        if ref_token["label"] == "B-A":
            ...
            same_index_in_src = [
                ...
                if src_token["index"] == ref_token["index"]
            ...
            if same_index_in_src:  # it is not an empty list
                ...
                if same_postag:
                    src_token[
                        "label"
                    ] = "B-R"  # the token in the source must be labeled as replace
                    ref_token[
                        "label"
                    ] = "O"  # the token in the reference now has no label
                    src_token["replace"] = [
                        ref_token["word"]
                    ]  # include the information of the replacement
        ...
        if token["label"] in old_token_labels:
            ...
            if parent:  # the token is inside one of the specified syntactic groups
                ...
                for gt in group_tokens:
                    if (
                        gt == token["word"]
                    ):  # check if the word in the group has been labeled
                        if token["label"] in old_token_labels:
                            with_same_label += 1
                        ptr_token += 1
                        if (
                            ptr_token == num_tokens
                        ):  # check that there are still tokens annotated
                            break
                        token = annots[ptr_token]
        ...
        if src_token["label"] in ["B-D", "B-DC", "I-DC", "I-D", "I-R"]:
            ...
            if (src_token["index"] - shift_left + shift_right) != ref_index:
                if src_token["label"] == "O":  # the token is kept and moved
                    ...
                    new_label = src_token[
                        "label"
                    ]  # in any other case, the label stays the same
    ...
    for token_index, token_word in enumerate(src, start=1):
        ...
        if token_word in string.punctuation:
            ...
            if aligns_list:
                ...
                if (
                    len(aligns_list) == 1
                    and token_word.lower() == ref[aligns_list[0] - 1].lower()
                ):
                    # it is a 'keep'
                    src_token["label"] = "O"
                else:
        ...
        if token_word in string.punctuation:
            ...
            if aligns_list:
                ...
                if len(aligns_list) > 1:
                    ...
                    for i in range(
                        1, len(aligns_list)
                    ):  # token 0 already has 'B-R' because of label_delete_replace
                        src_index = aligns_list[i]
                        src_token = [
                            src_token
                            for src_token in src_annots
                            if src_token["index"] == src_index
                        ][0]
                        src_token["label"] = "I-R"
                        src_token[
                            "replace"
...
def annotate_sentence(
    src, ref, aligns, src_parse, ref_parse, include_phrase_level=False
):
    ...
    if include_phrase_level:
        ...
        _label_group_simop(
            src_annots, src_parse, CLAUSE_SYNT_TAGS, ["B-D", "I-D"], "DC"
        )
        _label_group_simop(
            ref_annots, ref_parse, CLAUSE_SYNT_TAGS, ["B-A", "I-A"], "AC"
        )
        ...
        # clause-level move
        _label_group_simop(
            src_annots, src_parse, CLAUSE_SYNT_TAGS, ["B-M", "I-M"], "MC"
        )
...
class WordOperationAnnotator:
    ...
    def analyse_operations(
        ...
        if as_str:
            score_per_label = " ".join(
                [
                    f"{label}={100. * score:.2f}"
    ...
    def compute_operations_sentence_scores(
        ...
        all_simops_orig_refs, _ = self.identify_operations(
            all_orig_sents, all_ref_sents
        )
    ...
    def identify_operations(self, orig_sentences: List[str], simp_sentences: List[str]):
        ...
        for orig_sent, simp_sent, orig_parse, simp_parse in tqdm(
            ...
            word_aligns_orig_simp = self._get_word_alignments(
                orig_sent, orig_parse, simp_sent, simp_parse
            )
    ...
    def _get_word_alignments(
        self, orig_sent: str, orig_parse, sys_sent: str, sys_parse
    ):
